...
3. Create the jaas configuration in the self-provisioning plugin UI.
   ...
   7. In the **Admin username** field, specify a user that you want to add to the broker and assign to the broker admin role. By default, the user you specify is added to the admin and queue_users roles on the broker. Initially, this is the only user that has permission to use the broker for messaging.
...
4. Update the permissions for the new roles in the **extra-roles.properties** file you added to the JAAS configuration:
