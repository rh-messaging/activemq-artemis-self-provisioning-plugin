...
import { totalProducedQuery } from '../../utils/queries';
  ...
  const queries = useMemo(
    () =>
      [...Array(state.size)].map((_, i) =>
        totalProducedQuery(state.name, state.namespace, i),
      ),
    ...
    <>
      ...
      <MetricsErrorBoundary>
        <CardQueryBrowser
          ...
          title={t('Total Produced')}
          helperText={t('Total Produced')}
          dataTestId={'metrics-broker-total-produced'}
          yTickFormat={yTickFormat}
          label={'\n\n\n\n' + t('Messages')}
          ariaTitle={t('Total Produced')}
          error={errorObject}
        />
      </MetricsErrorBoundary>
    </>
